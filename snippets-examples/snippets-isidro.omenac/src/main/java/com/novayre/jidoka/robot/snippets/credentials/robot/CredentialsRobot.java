package com.novayre.jidoka.robot.snippets.credentials.robot;
...
public class CredentialsRobot implements IRobot {
	...
	public void loginApp() {
		...
		try {
			...
			server.info(String.format("Using the credentials 1 with USER: %s and PASSWORD: %s", credentials1.getUsername(),
					credentials1.getPassword()));

			server.info(String.format("Using the credentials 2 with USER: %s and PASSWORD: %s", credentials2.getUsername(),
					credentials2.getPassword()));

			server.info(String.format("Using the credentials 3 with USER: %s and PASSWORD: %s", credentials3.getUsername(),
					credentials3.getPassword()));
		} catch (Exception e) {
