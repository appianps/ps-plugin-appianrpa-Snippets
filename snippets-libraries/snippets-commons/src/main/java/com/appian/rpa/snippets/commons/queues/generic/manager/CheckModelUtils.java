...
/**
 * Class to check if the given class meet the requirements for the library to
 * work
 *
 */
public class CheckModelUtils {
	...
	public static void checkFieldsWithAnnotations(Class<?> modelClass) {
		...
		if (fieldsKey.isEmpty()) {
			...
			if (fieldsKey.get(0).getAnnotationsByType(AItemField.class).length == 0) {
				throw new JidokaFatalException(
						"The key field must also have the annotation @" + AItemField.class.getSimpleName());
			}
		}
