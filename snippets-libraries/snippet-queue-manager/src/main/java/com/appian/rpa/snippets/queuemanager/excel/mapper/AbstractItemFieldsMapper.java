package com.appian.rpa.snippets.queuemanager.excel.mapper;
...
import com.appian.rpa.snippets.queuemanager.excel.annotations.utils.ColumnAtEndCreator;
...
public abstract class AbstractItemFieldsMapper<T> implements IRowMapper<IExcel, T> {
	...
	public boolean isLastRow(T row) {
		...
		String keyValue = AnnotationUtils.getKeyFieldValue(row);
	...
	public T map(IExcel data, int rowNum) {
		...
		List<Field> queueFields = AnnotationUtils.getFieldsWithAnnotation(getTClass(), AItemField.class);
	...
	public void update(IExcel data, int rowNum, T row) {
		...
		List<Field> fields = AnnotationUtils.getFieldsWithAnnotation(getTClass(), AItemField.class);
		...
		for (Field field : fields) {
			...
			Object value = AnnotationUtils.getFieldValue(row, field);
	...
	private void initTitlePositionMap() {
		...
		List<Field> excelFields = AnnotationUtils.getFieldsWithAnnotation(getTClass(), AItemField.class);
	 ...
	 */
	private String getFieldNameRegExp(AItemField annotation) {
